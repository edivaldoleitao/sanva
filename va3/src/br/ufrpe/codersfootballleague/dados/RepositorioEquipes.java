                ...
                if (!qualquer.getNome().equals(e.getNome())) {
                    for (String jE : e.getJogadores().values()) {
                        if (qualquer.getJogadores().values().contains(jE)) {
                            throw new EquipeException("Jogador " + jE + " já faz parte do time " + qualquer.getNome());
                        }
                    }
                } else {
                    throw new EquipeException("Equipe com mesmo nome já existe");
                }
            }
            equipes.add(e);
            salvar();
        } else {
            throw new EquipeException("Equipe não selecionada");
        }
            ...
            if (!e.getNome().equals(antiga)) {
                for (String jE : nova.getJogadores().values()) {
                    if (e.getJogadores().values().contains(jE)) {
                        throw new EquipeException("Jogador " + jE + " já faz parte do time " + e.getNome());
                    }
                }
            } else {
